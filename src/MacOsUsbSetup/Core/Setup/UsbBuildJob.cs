    ...
    private static string InstallReadme(InstallPlan plan, InstallAssistantInfo info) =>
        $"""
         ...
         2. Menüleiste: Dienstprogramme -> Terminal öffnen. EIN Kommando:
            ...
            keine manuelle Formatierung, keine Rückfragen (10-s-Countdown, Abbruch
            mit Strg-C). Mehrere interne Platten? Ziel angeben:
              bash "/Volumes/{DataLabelForReadme}/offline-install.command" /dev/disk0

            Fallback, falls das Kommando auf deinem Gerät hakt (interaktiv):
              cd "/Volumes/{DataLabelForReadme}" && bash UnPlugged.command
            (dann vorher im Festplattendienstprogramm die interne Platte als
            APFS "Macintosh HD" löschen)
