...
# Findet den Voll-Installer, formatiert die interne Platte automatisch und installiert.
# Optionales Argument: Zielplatte (z. B. /dev/disk0); ohne Argument automatische Erkennung.
set -o pipefail
say() { printf '%s\n' "$*"; }
...
if [ ! -f "$pkg" ]; then
  ...
  say "   Nur bei Sonoma/Sequoia noetig - Datenpartition manuell mounten:"
...
# 4. Formatieren (APFS / GUID) - keine manuelle Formatierung noetig.
